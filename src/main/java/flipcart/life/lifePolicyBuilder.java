 ...
 * 
 */
public class lifePolicyBuilder implements PolicyBuilder<lifePolicy> {

	@Override
    public boolean build(Endpoint endpoint, BuilderContext context) {
        List<lifePolicy> polices = getPolicies(endpoint);
    ...
    public boolean build(EndpointReference endpointReference, BuilderContext context) {
        List<lifePolicy> polices = getPolicies(endpointReference);
    ...
    public boolean build(Component component, Implementation implementation, BuilderContext context) {
        List<lifePolicy> polices = getPolicies(implementation);
    ...
    private List<lifePolicy> getPolicies(PolicySubject subject) {
        List<lifePolicy> polices = new ArrayList<lifePolicy>();
        for (PolicySet ps : subject.getPolicySets()) {
            for (PolicyExpression exp : ps.getPolicies()) {
                if (getPolicyType().equals(exp.getName())) {
                    polices.add((lifePolicy)exp.getPolicy());
                }
            }
        }
        return polices;
